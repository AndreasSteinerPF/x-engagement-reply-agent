...
- Throttling is in-process (`p-limit`/`p-throttle`) around the X client (respect rate-limit headers, stop processing remaining authors on 429 rather than crashing) and around Bedrock calls (bounded concurrency ~3-4).
...
- One integration test running `runMonitor()` end-to-end against faked X/MCP/Bedrock/Asana clients, using an extended version of `examples/reference/fixtures/synthetic-post-and-reply.json`.
- `cdk synth` test asserting least-privilege IAM (table scoped, Bedrock invoke, Secrets read scoped to specific secrets).
- `scripts/invoke-local.ts` (via `tsx`): `--author <handle> --dry-run [--live-mcp] [--live-llm] [--force-retask]` — runs the pipeline in-process without deploying; mirrors the reference's `?dryRun=true&author=` query-param pattern as a CLI, since this agent is schedule-only (no HTTP route). `--force-retask` bypasses the dedupe read for one invocation (still goes through the live Asana lookup) so the "edit a prompt, show updated draft" and "add a 6th prompt" demo scenarios don't require waiting for a genuinely new post against an already-tasked one.
...
5. **Asana tasking** — Asana client, side-effect gateway, full orchestration wiring, `handler.ts`. *Verify: live run against a real sandbox Asana project creates the expected parent+subtasks; a repeat run creates zero duplicates.*
