...
- **Implemented (second post-Phase-5 audit, prompted by re-checking the README's acceptance criteria after the pipeline was already live-verified):** watchlist batch rotation. `defaultBatchSize` was validated by `config/settings.ts` from Phase 1 onward but never actually read anywhere -- `runMonitor` processed every active author every run regardless of the configured batch size, with a code comment explicitly calling this out as "intentionally not implemented yet ... a documented no-op simplification" at demo scale. That comment undersold the gap: the README's acceptance criteria explicitly requires "batching across the watchlist using configurable batch size and cursor rotation," not just at scale. Fixed by porting the reference implementation's own approach (investors-mcp's `monitor-x` route: a single persisted `state.cursor` integer, a batch of `min(batchSize, activeAuthors.length)` authors selected starting at that cursor and wrapping cyclically via modulo, advanced after each run by however many authors were *actually attempted* -- not the full batch size, so an early stop on a rate limit doesn't skip over unattempted authors). New `src/state/rotation-store.ts` (`RUN#ROTATION` / `STATE` / `cursorIndex`, matching the state design table below) plumbed through `SideEffectGateway.writeRotationCursor` (write path, dry-run-gated like every other write) and a new `RunMonitorDeps.rotationStore` (read path, matching the existing `cursorStore`/`dedupeStore` split). `scripts/invoke-local.ts` gets a fresh in-memory rotation store per invocation, never persisted -- rotation is a genuine no-op there anyway, since `--author` always pre-filters the watchlist to one entry before calling `runMonitor`, and a batch over a one-element list always selects that same element regardless of cursor value, mirroring the reference's own `authorFilter` bypass with no special-casing needed.
...
- **Not yet implemented, deliberately deferred:** `WatchedAuthor.aliases` (`handles`/`authors`) is validated by `src/config/watchlist.ts` and required by the README's acceptance criteria ("author name, handle, company, aliases, and active flag"), but nothing in the pipeline reads it yet -- found during a post-Phase-5 audit. Two distinct pieces, different sizes:
  - `aliases.authors` -- widen `createAsanaTasking`'s excluded-author check to also match against these display-name variants, not just the primary `author` name. Small, contained (a few lines in `create-asana-tasking.ts` plus threading the list through from `run-monitor.ts`).
  - `aliases.handles` -- also poll these handles under the same watched-author identity. A real restructuring, not a one-liner: `processAuthor` needs to loop over `[handle, ...aliases.handles]`, track a **separate cursor per underlying handle** (different X accounts have independent status-ID sequences -- `CursorStore` already keys by an arbitrary handle string, so no store changes needed, just one call per handle instead of one per author), merge the resulting posts before processing, and still attribute tasks/notes to the one shared author/company identity. Also needs `loadWatchlist`'s duplicate-handle check extended so an alias handle can't silently collide with another entry's primary handle or its own aliases -- today that validation only compares primary handles against each other.
  - Sequencing when this is picked back up: `aliases.authors` first (quick, low risk), `aliases.handles` as its own deliberate pass.
